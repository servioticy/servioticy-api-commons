...
public class ChannelsMapper {
    ...
    public static JsonNode parseCreation(JsonNode root) throws JsonParseException, JsonMappingException, IOException {

        Map<String, JsonNode> channels = null;
        ...
        for (Map.Entry<String, JsonNode> channel : channels.entrySet()) {
            ...
            if (channel.getValue().path("type").isMissingNode()) {
                ...
                if (Arrays.asList(Types).contains(channel.getValue().get("type").asText().toLowerCase()) == false) {
                    throw new ServIoTWebApplicationException(Response.Status.BAD_REQUEST, "Channel " + channel.getKey()
                            + " with incorrect type: " + channel.getValue().get("type").asText());
                }
                // Enforcement location
                if (channel.getKey().equals("location")) {
                    if (!channel.getValue().get("type").asText().toLowerCase().equals("geo_point")) {
                        throw new ServIoTWebApplicationException(Response.Status.BAD_REQUEST,
                                "location channel has to be geo_point type");
                    }
                }
            }
        }
    ...
    public static JsonNode parsePutData(JsonNode stream, JsonNode root)
            throws ServIoTWebApplicationException, JsonParseException, JsonMappingException, IOException {
        Map<String, JsonNode> channels;
        channels = mapper.readValue(root.traverse(), new TypeReference<Map<String, JsonNode>>() {
        });

        for (Map.Entry<String, JsonNode> channel : channels.entrySet()) {
            // Check DB integrity -> TODO to delete when all the created data is
            // checked
