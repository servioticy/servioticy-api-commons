...
public class SO {
     ...
     * @param body
     */
    public SO(String authToken, String userId, String body) {
        ...
        if (root.path("dataTTL").isMissingNode()) {
            ...
            if (!root.path("dataTTL").isInt()) {
                throw new ServIoTWebApplicationException(Response.Status.BAD_REQUEST,
                        "The dataTTL has to be an Integer");
            }
        ...
        // If is a CSO with groups field create the derived subscriptions
        if (!root.path("groups").isMissingNode()) {
            createGroupsSubscriptions(authToken, root.get("groups"));
        }
    }
     ...
     *
     * @param JsonNode
     *            representing groups information
     */
    public void createGroupsSubscriptions(String authToken, JsonNode root) {
        ArrayList<Group> agroups = new ArrayList<Group>();

        try {
            Map<String, JsonNode> groups = mapper.readValue(root.traverse(),
        ...
        for (Group group : agroups) {
            group.createSubscriptions(authToken, soId, userId);
        }
    ...
    public String responseInternalSubscriptions(String streamId, boolean externalOnly) {
        ...
        try {
            for (String id : IDs) {
                Subscription tmp = CouchBase.getSubscription(id);
                if (tmp != null)
                    subsArray.add(mapper.readTree(tmp.getString())); // In raw
                                                                     // format
                else
                    LOG.error("Subscription id: " + id
                            + ", reported by search engine but not found in CouchBase. Skipping...");
